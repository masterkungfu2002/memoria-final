...
export default async function AlbumPage({
  ...
  const res = await fetch(new URL(`/api/album/${encodeURIComponent(id)}`, baseUrl), {
    cache: "no-store",
  });
  ...
  if (res.ok) {
    ...
    return (
      <>
        ...
        <MemoraQrExperience album={album} initialProfile={resolveInitialProfile(album)} />
      </>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl text-center space-y-3">
        <p className="font-serif text-2xl">Could not open this memory</p>
